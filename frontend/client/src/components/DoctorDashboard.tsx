...
import ActiveConsultationFlow from "./consultation/ActiveConsultationFlow";
...
export default function DoctorDashboard({
  ...
  const renderPatientProfile = () => {
    ...
    if (profileMode === "consult") {
      const patientAssessmentAnswers = prakritiQuestions.map((question, index) => ({
        question: question.label,
        answer: String(resolvedAssessmentAnswers[index] || "Not specified"),
      }));

      const initialDiagnosis = {
        finalPrakriti: String(latestAssessmentPayload?.primaryDosha || selectedPatient.prakriti || ""),
        finalVikriti: String(latestAssessmentPayload?.doctorNotes || selectedPatient.vikriti || ""),
        chiefComplaint: String(selectedAppointment?.diagnosis || selectedPatient.issues?.join(", ") || ""),
      };

      return (
        <div className="relative mx-auto max-w-[1500px] rounded-2xl border border-gray-200 bg-white p-3 shadow-sm sm:p-6 lg:p-7">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">{selectedPatient.name}</h2>
              <p className="mt-1 text-sm text-slate-600">
                Active consultation flow for appointment-driven treatment planning
              </p>
            </div>
          ...
          <ActiveConsultationFlow
            ...
            prakritiScores={{
              vata: Number(resolvedPrakritiScores.vata || 0),
              pitta: Number(resolvedPrakritiScores.pitta || 0),
              kapha: Number(resolvedPrakritiScores.kapha || 0),
            }}
            patientProfileSummary={{
              age: Number(selectedPatient.age || 0),
              gender: String(selectedPatient.gender || "-"),
              prakriti: String(selectedPatient.prakriti || "Not Assessed"),
              priority: String(selectedPatient.priority || "low"),
              email: String(selectedPatient.email || "-"),
              phone: String(selectedPatient.phone || "-"),
              issues: Array.isArray(selectedPatient.issues) ? selectedPatient.issues : [],
              compliance: Number(selectedPatient.compliance || 0),
            }}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 mb-8">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-3 w-full sm:w-auto sm:justify-end">
            ...
            <span className={`px-3 py-2.5 rounded-lg text-xs font-semibold inline-flex items-center justify-center ${
                  (profileMode as "view" | "consult") === "consult" ? "bg-[#1F5C3F] text-white" : "bg-gray-100 text-gray-700"
                }`}>
                  {(profileMode as "view" | "consult") === "consult" ? "Consultation Mode" : "View Mode"}
                </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-7 xl:gap-7 items-start">
          ...
          <div className="xl:col-span-8 2xl:col-span-9 space-y-7">
            <div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-5 md:p-6">
              ...
              {(profileMode as "view" | "consult") === "consult" ? (
